...
def get_image_pairs():
    image_pairs = []
    im1 = np.random.rand(200, 200).astype(np.float64)
    im2 = im1.copy()
    im2 = rotate_image(im2,-1)

    image_pairs.append((im1,im2))

    im1 = "data/collision/00000001.jpg"
    im2 = "data/collision/00000002.jpg"
    im1 = cv2.imread(im1,cv2.IMREAD_GRAYSCALE)/255
    im2 = cv2.imread(im2,cv2.IMREAD_GRAYSCALE)/255
    image_pairs.append((im1, im2))

    im1 = "data/collision/00000181.jpg"
    im2 = "data/collision/00000182.jpg"
    im1 = cv2.imread(im1,cv2.IMREAD_GRAYSCALE)/255
    im2 = cv2.imread(im2,cv2.IMREAD_GRAYSCALE)/255
    image_pairs.append((im1, im2))

    im1 = "data/lab2/005.jpg"
    im2 = "data/lab2/006.jpg"
    im1 = cv2.imread(im1,cv2.IMREAD_GRAYSCALE)/255
    im2 = cv2.imread(im2,cv2.IMREAD_GRAYSCALE)/255
    image_pairs.append((im1, im2))

    im1 = "data/disparity/cporta_left.png"
    im2 = "data/disparity/cporta_right.png"
    im1 = cv2.imread(im1,cv2.IMREAD_GRAYSCALE)/255
    im2 = cv2.imread(im2,cv2.IMREAD_GRAYSCALE)/255
    image_pairs.append((im1, im2))
